<template>
    <div class="f-daipingjia-con">
        <div class="f-daipingjia-order" v-if="number == 0">
        ...
        <div v-else class="f-daipingjia-info">
            <div class="f-daifahuo-shop" :key="sid" v-for="(shop,sid) in AllorderInfo">
                ...
                <div class="f-tixingfahuo">
                    <span>再次购买</span><span @click="reprice(shop.order_id)">申请退款</span>
                </div>
            </div>
        </div>
        <f-loading></f-loading>
    ...
    export default {
        ...
        methods:{
            _initAllOrderInfo(){
                ...
                fetch('http://119.3.133.207:5000/delivery/good/?token='+token).then(res=>{
                    res.json().then(data => {
                        console.log(data)
            ...
            reprice(id){
                let token = localStorage.getItem("token")
                fetch('http://119.3.133.207:5000/order/refund/',{
                    method:"POST",
                    mode: "cors",
                    headers:{
                    ...
                    body:JSON.stringify({order_id:id, reson: "",token:token})
                }).then(res=> {
                    return res.json();
                }).then(data=>{
                    console.log(data);
